...
- Chapter 7: ✅ drafted (fast-doubling Fibonacci, O(log n) — the recursive `fast_fib` verified against `fib`; both doubling identities, overflow, and termination)

> **Run with `--lean-backend`** (see [Chapter 0](chapters/00-setup/README.md) and the Setup snippet above). It's the supported invocation; chapters 0–7 verify with `0 errors` under it.
